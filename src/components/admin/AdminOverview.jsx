...
export default function AdminOverview({ stats }) {
  ...
  const topStats = [
    { label: "Total users", value: metrics.visibleUsers.length, icon: Users, tone: "bg-blue-50 text-blue-700" },
    { label: "New in 24h", value: metrics.joinedToday.length, icon: UserPlus, tone: "bg-emerald-50 text-emerald-700" },
    { label: "Active paid", value: metrics.activeSubscriptions.length, icon: CheckCircle2, tone: "bg-green-50 text-green-700" },
    { label: "Due / missing", value: metrics.dueSubscriptions.length, icon: ShieldAlert, tone: "bg-red-50 text-red-700" },
    { label: "Renewal soon", value: metrics.renewalSoonUsers.length, icon: CalendarClock, tone: "bg-amber-50 text-amber-800" },
    { label: "Total revenue", value: formatMoney(metrics.totalRevenue), icon: DollarSign, tone: "bg-violet-50 text-violet-700" },
  ];
...
const StatusPanel = ({ title, icon: Icon, users, tone }) => (
